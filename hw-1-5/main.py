...
def main(argv):
    ...
    y_size = gray_pic1.shape[0] // 2
    x_size = gray_pic1.shape[1] // 2

    gray_pic1 = cv2.resize(gray_pic1, (x_size, y_size))
    gray_pic2 = cv2.resize(gray_pic2, (x_size, y_size))

    keypoints1, descriptor1 = get_keypoint_descriptor(gray_pic1)
    keypoints2, descriptor2 = get_keypoint_descriptor(gray_pic2)

    pic1_with_key_points = cv2.drawKeypoints(gray_pic1, keypoints1, None)
    pic2_with_key_points = cv2.drawKeypoints(gray_pic2, keypoints2, None)
    ...
    if len(match_picture) > 4:
        points1 = np.float32([keypoints1[i.queryIdx].pt for i in match])
        points2 = np.float32([keypoints2[i.queryIdx].pt for i in match])
